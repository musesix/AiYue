...
# ai_yue_pro.py
# New Name: AiYue_Pro
# Features: IDOR Detection + AI Analysis + Data Masking + Sorting + Copy + Default Filters
...
from java.awt import BorderLayout, Dimension, GridBagLayout, GridBagConstraints, Insets, Color, Font, Toolkit
...
from javax.swing import (JSplitPane, JTabbedPane, JPanel, JLabel, JTable,
                         ...
                         SwingConstants, ToolTipManager, RowSorter, SortOrder, JPopupMenu, JMenuItem)
from javax.swing.table import AbstractTableModel, DefaultTableCellRenderer, TableRowSorter
from javax.swing.event import ListSelectionListener
from java.util import ArrayList, HashSet, Comparator
...
class AiYueUI:
    ...
    def _init_ui(self):
        ...
        # Listeners
        self.table.getSelectionModel().addListSelectionListener(TableSelectionHandler(self.table, self._controller))
        self.table.addMouseListener(TableRightClickMenu(self.table, self.table_model, self._controller._log))
        ...
        # Button Layout: Move Clear button to left
        c.gridy = 1
        ...
        c.gridy = 7;
        self.txt_filter_path = JTextField("/dict,.js,.css,.html,.htm,.png,.jpg,.jpeg,.gif,.webp,.svg,.ico,.woff,.woff2,.ttf,.mp4,.mp3,.json,.xml,.map,.pdf,.zip");
        ...
        # --- [MODIFIED START] 3-Column Layout Implementation ---
        
        # Helper: Creates a Panel with Titled Border containing Response/Request tabs
        def create_column_viewer(title, req, res):
            # Inner Tabs: Default Response
            tabs = JTabbedPane()
            tabs.addTab("Response", res.getComponent())
            tabs.addTab("Request", req.getComponent())
            tabs.setSelectedIndex(0)
            
            # Wrapper Panel with Title
            wrapper = JPanel(BorderLayout())
            wrapper.setBorder(BorderFactory.createTitledBorder(title))
            wrapper.add(tabs, BorderLayout.CENTER)
            return wrapper

        # 1. Create the 3 columns
        col_origin = create_column_viewer(L_TAB_ORIGIN, self.editor_origin_req, self.editor_origin_res)
        col_low = create_column_viewer(L_TAB_LOW, self.editor_low_req, self.editor_low_res)
        col_unauth = create_column_viewer(L_TAB_UNAUTH, self.editor_unauth_req, self.editor_unauth_res)

        # 2. Arrange them using Nested JSplitPanes
        # Inner Split: Low | Unauth
        split_inner = JSplitPane(JSplitPane.HORIZONTAL_SPLIT, col_low, col_unauth)
        split_inner.setResizeWeight(0.5) # Divide equally

        # Outer Split: Origin | (Low | Unauth)
        split_outer = JSplitPane(JSplitPane.HORIZONTAL_SPLIT, col_origin, split_inner)
        split_outer.setResizeWeight(0.33) # Origin takes 1/3, rest takes 2/3

        # 3. Main Bottom Tabs (Data Comparison vs AI Result)
        bottom_tabs = JTabbedPane()
        bottom_tabs.addTab(u"\u6570\u636e\u5bf9\u6bd4 (Data Comparison)", split_outer) # Tab 1
        bottom_tabs.addTab(L_TAB_AI_RESULT, ai_scroll)                              # Tab 2
...
class LogEntry:
    def __init__(self, id, url, method, origin_rr):
        ...
        self._len_origin = 0;
        # For sorting: use Integers. -1 = Pending/Loading, -2 = Error
        self._len_low = -1; 
        self._len_unauth = -1
        self._ai_result = "";
        self._ai_detail = ""
...
class CustomTableModel(AbstractTableModel):
    ...
    def getColumnClass(self, col):
        if col == COL_ID or col == COL_LEN_ORIG or col == COL_LEN_LOW or col == COL_LEN_UNAUTH:
            return Integer
        return String
    ...
    def getValueAt(self, row, col):
        ...
        e = self._log.get(row)
        if col == COL_ID: return e._id
        if col == COL_METHOD: return e._method
        if col == COL_URL: return e._url.toString()
        if col == COL_LEN_ORIG: return e._len_origin
        if col == COL_LEN_LOW: return e._len_low
        if col == COL_LEN_UNAUTH: return e._len_unauth
...
class AnalysisColorRenderer(DefaultTableCellRenderer):
    ...
    def getTableCellRendererComponent(self, table, value, isSelected, hasFocus, row, col):
        c = super(AnalysisColorRenderer, self).getTableCellRendererComponent(table, value, isSelected, hasFocus, row, col)
        ...
        if col == COL_LEN_LOW or col == COL_LEN_UNAUTH:
            try:
                ...
                if val_int == -1:
                    ...
                    c.setText(str(val_int))
            except:
                pass
                
            try:
                model_row = table.convertRowIndexToModel(row)
                if model_row < self._log.size():
                    entry = self._log.get(model_row)
                    orig_len = entry._len_origin
                    target_len = val_int
                    
                    if target_len >= 0:
                        diff = abs(orig_len - target_len)
                        threshold = (orig_len * 0.15) + 50
                        if diff < threshold:
    ...
    def valueChanged(self, e):
        if not e.getValueIsAdjusting(): 
            ...
            if view_row != -1:
                ...
                self._controller.on_row_selected(model_row)


class TableRightClickMenu(MouseAdapter):
    def __init__(self, table, model, log):
        self._table = table
        self._model = model
        self._log = log
        self._menu = JPopupMenu()
        item_copy = JMenuItem(L_COPY_URL)
        item_copy.addActionListener(self.action_copy_url)
        self._menu.add(item_copy)

    def mousePressed(self, e):
        self.check_trigger(e)

    def mouseReleased(self, e):
        self.check_trigger(e)

    def check_trigger(self, e):
        if e.isPopupTrigger() and self._table.getSelectedRowCount() > 0:
            self._menu.show(e.getComponent(), e.getX(), e.getY())

    def action_copy_url(self, e):
        ...
        for view_row in rows:
            ...
            if model_row < self._log.size():
                entry = self._log.get(model_row)
                urls.append(entry._url.toString())
        
        if urls:
            content = "\n".join(urls)
            sel = StringSelection(content)
...
class BurpExtender(IBurpExtender, ITab, IHttpListener, IMessageEditorController, IExtensionStateListener):
    def registerExtenderCallbacks(self, callbacks):
        ...
        self._stdout.println("AiYue_Pro v3 (Default Filters) Loaded!")
    ...
    def on_row_selected(self, model_row):
        if model_row == -1 or model_row >= self._log.size(): return
        e = self._log.get(model_row);
